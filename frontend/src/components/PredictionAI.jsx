...
export function PredictionAI() {
  ...
  const handlePredict = async () => {
    ...
    try {
      ...
      // Format detail aktual
      const formattedActual = (data.detail_aktual || []).map((item) => {
        const d = new Date(item.tanggal);
        const day = d.getDate();
        return {
          date: `${day} ${monthNames[d.getMonth()]}`,
          actual: item.aktual_penjualan_porsi,
          predicted: null,
        };
      });

      // Format detail prediksi
      const formattedPredicted = data.detail_prediksi.map((item) => {
        const d = new Date(item.tanggal);
        const day = d.getDate();
        return {
          date: `${day} ${monthNames[d.getMonth()]}`,
      ...
      // Hubungkan garis aktual dan prediksi secara halus
      if (formattedActual.length > 0 && formattedPredicted.length > 0) {
        formattedActual[formattedActual.length - 1].predicted = formattedActual[formattedActual.length - 1].actual;
      }

      const combinedData = [...formattedActual, ...formattedPredicted];
            ...
            <div className="bg-white dark:bg-gray-800 p-6 rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm transition-colors duration-300">
              <div className="flex items-start justify-between mb-4">
                <div>
                  ...
                  <h2 className="text-3xl font-bold text-gray-900 dark:text-white">{metrics?.accuracy ?? '92.5'}%</h2>
                </div>
              ...
              <p className="text-sm text-gray-500 dark:text-gray-400 bg-gray-50 dark:bg-gray-700/50 inline-block px-2 py-1 rounded transition-colors">Rata-rata error (MAPE): <strong>{metrics?.mape ?? '7.5'}%</strong></p>
            </div>
            ...
            <div className="bg-white dark:bg-gray-800 p-6 rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm transition-colors duration-300">
              <div className="flex items-start justify-between mb-4">
                <div>
                  ...
                  <h2 className="text-3xl font-bold text-gray-900 dark:text-white">{metrics?.rmse ?? '18.2'}</h2>
                </div>
            ...
            <div className="bg-white dark:bg-gray-800 p-6 rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm transition-colors duration-300">
              <div className="flex items-start justify-between mb-4">
                <div>
                  ...
                  <h2 className="text-3xl font-bold text-gray-900 dark:text-white">{metrics?.r2 ?? '91.2'}%</h2>
                </div>
              ...
              <p className="text-sm text-gray-500 dark:text-gray-400 bg-gray-50 dark:bg-gray-700/50 inline-block px-2 py-1 rounded transition-colors">Koefisien Determinasi (R²)</p>
            </div>
          </div>
